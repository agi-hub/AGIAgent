...
def read_language_config(config_file: str = None) -> str:
    ...
    Args:
        config_file: Config file path, defaults to config/config.txt in current directory
        
    Returns:
        Language code ('en' or 'zh')
    """
    if config_file is None:
        # Calculate project root directory (2 levels up from this file)
        # From src/multi_round_executor/config.py -> AGIBot/
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(current_dir))
        config_file = os.path.join(project_root, "config", "config.txt")
