# AGIBot MCP (Model Context Protocol) 支持

AGIBot 现在支持 MCP (Model Context Protocol) 协议，可以与外部工具服务器进行通信，大大扩展了系统的工具生态。
...
### 2. 使用 MCP 工具
在 AGIBot 中，MCP 工具使用 `服务器名:工具名` 的格式调用：
...
### 调试模式
AGIBot 支持详细的调试日志，可以帮助诊断 MCP 相关问题：
