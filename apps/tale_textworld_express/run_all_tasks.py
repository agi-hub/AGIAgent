...
APP_NAME = "tale_textworld_express"
DATASET_NAME = "TextWorld-Express"
...
from src.tools.tale_textworld_express_tools import TaleTextworldExpressTools as ToolClass
...
def _requirement(env_name: str) -> str:
    return (
        f"只运行一个{DATASET_NAME}任务：{env_name}。"
        f"只允许调用 {TOOL_PREFIX}_action。"
        f"首次调用必须传 env_name=\"{env_name}\"。"
        f"之后每轮继续调用 {TOOL_PREFIX}_action(action=...)，直到返回 done=true。"
        "结束时输出 best_score/current_score/full_score/score_rate。"
    )
